...
select_log(){
	read -p "请输入需要截取的日志所在服务器(pcw|cad): " Server	
	...
	echo 截取日志另存为: $DIR/${Server}_apache_${StartDate:5:5}-${EndDate:5:5}.log
	echo $DIR/${Server}_apache_${StartDate:5:5}-${EndDate:5:5}.log > $DIR/.log
}
